...
def get_instance_model_optimizer(device, input_shape, learning_rate=0.0001, z_dims=2, pixel=64, ep=100):
	print(device)
	...
	scheduler_class = optim.lr_scheduler.LinearLR(class_optim, start_factor=1, end_factor=0, total_iters=ep)
	...
	scheduler_gen = optim.lr_scheduler.LinearLR(gen_optim, start_factor=1, end_factor=0, total_iters=ep)
	...
	scheduler_enc = optim.lr_scheduler.LinearLR(enc_optim, start_factor=1, end_factor=0, total_iters=ep)
	#discriminator = Discriminator(input_shape)
	dis_optim = torch.optim.Adam(discriminator.parameters(), lr=0.0005, betas=(0, 0.9))
	scheduler_dis = optim.lr_scheduler.LinearLR(dis_optim, start_factor=1, end_factor=0, total_iters=ep)
	
	return classifier, class_optim, generator, gen_optim, encoder, enc_optim, discriminator, dis_optim, scheduler_class, scheduler_gen, scheduler_enc, scheduler_dis 
